...
async function createManagedPaymentIntent({
    amount,
    currency = 'usd',
...
async function createStripeWebhookEvent(req, secret) {
    const stripe = getStripeClient();
    if (!stripe) {
        return null;
    }
    ...
    const rawBody = typeof req.text === 'function' ? await req.text() : req.body;
    return stripe.webhooks.constructEvent(rawBody, signature, secret);
}

async function pauseSellerPayouts(UserModel, seller, reason, holdDays = 14) {
    const stripe = getStripeClient();
