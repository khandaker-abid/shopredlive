...
function checkRateLimit({ req, name = 'default', windowMs = 60_000, max = 5 }) {
...
function updateKnownDevice(user, context) {
...
function loginRiskSummary(user, context) {
    const devices = normalizeKnownDevices(user);
    const currentFingerprint = context.deviceFingerprint;
    const knownDevice = devices.find((device) => device.fingerprint === currentFingerprint && device.trusted !== false);
...
function createMfaChallenge(user, context) {
...
function verifyMfaChallenge(user, challengeId, code) {
...
function issueCaptchaChallenge(purpose = 'auth') {
    const challengeId = crypto.randomUUID();
...
function verifyCaptchaChallenge({ challengeId, nonce, digest, purpose = 'auth' }) {
...
async function checkTransactionVelocity({ OrderModel, buyerId, amount, currency = 'USD', windowMinutes = 30, highValueThreshold = 75, maxHighValueOrders = 2 }) {
...
function detectTriangulationFraud({ shippingAddress = {}, ipGeo = {} }) {
...
async function placeSellerPayoutHold(UserModel, sellerId, { reason, days = 14 }) {
    const until = new Date(Date.now() + days * 24 * 60 * 60 * 1000);
    await UserModel.findByIdAndUpdate(sellerId, {
        $set: {
            'stripe.payoutsHeldUntil': until,
